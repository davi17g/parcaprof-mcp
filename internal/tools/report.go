...
func topHandler(c *parcaclient.Client) func(context.Context, *mcp.CallToolRequest, reportInput) (*mcp.CallToolResult, any, error) {
	return func(ctx context.Context, _ *mcp.CallToolRequest, args reportInput) (*mcp.CallToolResult, any, error) {
		if args.Query == "" {
		...
		rows := buildTopRows(nodes, limit, denom)
...
func buildTopRows(nodes []*querypb.TopNode, limit int, denom float64) []topRow {
...
func flamegraphHandler(c *parcaclient.Client) func(context.Context, *mcp.CallToolRequest, reportInput) (*mcp.CallToolResult, any, error) {
	return func(ctx context.Context, _ *mcp.CallToolRequest, args reportInput) (*mcp.CallToolResult, any, error) {
		if args.Query == "" {
			return errResult(fmt.Errorf("query is required")), nil, nil
		}
		...
		rows := aggregateFlamegraph(fg.GetRoot().GetChildren(), total, limit)
		return jsonResult(map[string]any{
			"unit":   fg.GetUnit(),
			"total":  total,
			"height": fg.GetHeight(),
...
func aggregateFlamegraph(roots []*querypb.FlamegraphNode, total int64, limit int) []flamegraphRow {
	...
	rows := make([]flamegraphRow, 0, len(agg))
	for name, c := range agg {
		rows = append(rows, flamegraphRow{Function: name, Cumulative: c, Pct: float64(c) / denom * 100})
	}
